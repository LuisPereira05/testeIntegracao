...
public class Carro {
    ...
    public void acelerar() {
        ...
        if (sistemaC.verificarNivel() <= 0) {
            System.out.println("ERRO: Sem combustivel");
            return;
        }

        // Liga motor se estiver desligado
        if (!motor.isLigado()) {
            motor.ligarMotor();
        }

        // Reduz nível da bateria
        sistemaE.reduzirCarga(2.0);

        // Aumenta marcha
        transmissao.aumentarMarcha();

        // Ajusta suspensão de acordo com a marcha atual
        suspensao.ajustarPorVelocidade(transmissao.getSistemaDeTransmissao().getEstado());

        // Desgasta pneus
        pneus.desgastar(1.0);

        // Gasta combustível
        sistemaC.consumir(1.5);

        // Recarrega bateria
        sistemaE.recarregar(1.0);

        // Trava portas automaticamente
    ...
    public static void main(String[] args) {
        ...
        Carro carro = new Carro("ML", 1999, "Vermelho", "C4RR1NH0", 0,
                sistemaC, sistemaE, sistemaT, motor, freios, luzes, painel, banco, pneus, portas, suspensao, transmissao);

        portas.get(0).destravar(chave);
        System.out.println(sistemaC.abastecer(70.0));

        portas.get(0).abrir(motor); // CORRIGIDO: adiciona parâmetro motor
